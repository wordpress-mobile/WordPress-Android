        ...
        for (version in oldVersion..newVersion) {
            when (version) {
                ...
                24 -> migrate(version) {
                    ...
                    db.execSQL(
                            "CREATE TABLE SitePluginModel (_id INTEGER PRIMARY KEY AUTOINCREMENT,LOCAL_SITE_ID INTEGER,"
                                    + "NAME TEXT,DISPLAY_NAME TEXT,PLUGIN_URL TEXT,VERSION TEXT,SLUG TEXT,DESCRIPTION TEXT,"
                    ...
                    db.execSQL(
                            "CREATE TABLE WPOrgPluginModel (_id INTEGER PRIMARY KEY AUTOINCREMENT,AUTHOR_AS_HTML TEXT,"
                ...
                30 -> migrate(version) {
                    ...
                    db.execSQL("CREATE TABLE IF NOT EXISTS ActivityLog (_id INTEGER PRIMARY KEY AUTOINCREMENT,LOCAL_SITE_ID "
                ...
                39 -> migrate(version) {
                    db.execSQL("DROP TABLE IF EXISTS QuickStartModel")
                    db.execSQL(
                            "CREATE TABLE IF NOT EXISTS QuickStartTaskModel (_id INTEGER PRIMARY KEY AUTOINCREMENT,"
                                    + "SITE_ID INTEGER,TASK_NAME TEXT,IS_DONE INTEGER,IS_SHOWN INTEGER)"
                    )
                    db.execSQL(
                            "CREATE TABLE IF NOT EXISTS QuickStartStatusModel (_id INTEGER PRIMARY KEY AUTOINCREMENT,"
                                    + "SITE_ID INTEGER,IS_COMPLETED INTEGER,IS_NOTIFICATION_RECEIVED INTEGER)"
                    )
                }
                40 -> migrate(version) {
                ...
                43 -> migrate(version) {
                    db.execSQL(
                            "CREATE TABLE ListModel (LAST_MODIFIED TEXT,DESCRIPTOR_UNIQUE_IDENTIFIER_DB_VALUE INTEGER,"
                                    + "DESCRIPTOR_TYPE_IDENTIFIER_DB_VALUE INTEGER,STATE_DB_VALUE INTEGER,"
                                    + "_id INTEGER PRIMARY KEY AUTOINCREMENT)"
                    )
                    db.execSQL(
                            "CREATE TABLE ListItemModel (LIST_ID INTEGER,REMOTE_ITEM_ID INTEGER,_id INTEGER PRIMARY KEY "
