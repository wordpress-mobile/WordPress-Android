...
@Inject constructor(appContext: Context, dispatcher: Dispatcher, requestQueue: RequestQueue,
    ...
    fun fetchActivity(site: SiteModel, number: Int, offset: Int) {
        ...
        val request = buildGetRequest(
                ...
                { response ->
                    ...
                    mDispatcher.dispatch(ActivityLogActionBuilder.newFetchedActivitiesAction(payload))
                },
                { networkError ->
    ...
    fun fetchActivityRewind(site: SiteModel, number: Int, offset: Int) {
        ...
        val request = buildGetRequest(
                ...
                { response ->
                    val payload = buildRewindStatusPayload(response, site, number, offset)
                    mDispatcher.dispatch(ActivityLogActionBuilder.newFetchedRewindStateAction(payload))
                },
                { networkError ->
    ...
    private fun buildRewindStatusPayload(response: RewindStatusResponse, site: SiteModel, number: Int, offset: Int):
        ...
        if (response.state == null) {
            return buildErrorPayload(site, number, offset, RewindStatusErrorType.MISSING_STATE)
        }
        if (response.restoreResponse != null && response.restoreResponse.rewind_id == null) {
            return buildErrorPayload(site, number, offset, RewindStatusErrorType.MISSING_RESTORE_ID)
        }
        if (response.restoreResponse != null && response.restoreResponse.status == null) {
            return buildErrorPayload(site, number, offset, RewindStatusErrorType.MISSING_RESTORE_STATUS)
        }
        if (RewindStatusModel.State.fromValue(response.state) == null) {
            return buildErrorPayload(site, number, offset, RewindStatusErrorType.INVALID_REWIND_STATE)
        }
    ...
    private fun buildErrorPayload(site: SiteModel, number: Int, offset: Int, errorType: RewindStatusErrorType) =
    ...
    private class ActivitiesResponse(val totalItems: Int?,
                                     val summary: String?,
                                     val current: Page?) {
        class Page(val orderedItems: List<ActivityResponse>)
        class ActivityResponse(val summary: String?,
                               val content: Content?,
                               val name: String?,
                               val actor: Actor?,
                               val type: String?,
    ...
    class RewindStatusResponse(val reason: String,
                               val state: String?,
                               val last_updated: Date,
                               val restoreResponse: RestoreStatusResponse?) {
        class RestoreStatusResponse(val rewind_id: String?,
                                    val status: String?,
                                    val progress: Int = 0,
                                    val message: String?,
