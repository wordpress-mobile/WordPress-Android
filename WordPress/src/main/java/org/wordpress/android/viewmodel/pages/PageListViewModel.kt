...
class PageListViewModel @Inject constructor(
    ...
    private fun refreshEditorTheme() {
        ...
        editorThemeStore.getIsBlockBasedTheme(pagesViewModel.site)?.let {
            isBlockBasedTheme.value = it
        }
        // Dispatch action to refresh the value from the remote
    ...
    fun onEditorThemeChanged(event: OnEditorThemeChanged) {
        if (pagesViewModel.site.id == event.siteId) {
            event.editorTheme?.themeSupport?.isBlockBasedTheme?.let {
                isBlockBasedTheme.value = it
            }
        }
    }
