...
import org.wordpress.android.analytics.AnalyticsTracker.Stat.READER_GLOBAL_RELATED_POST_CLICKED
import org.wordpress.android.analytics.AnalyticsTracker.Stat.READER_LOCAL_RELATED_POST_CLICKED
...
class ReaderPostDetailFragment : Fragment(),
    ...
    override fun onAttach(context: Context?) {
        ...
        toolbarHeight = activity!!.resources.getDimensionPixelSize(R.dimen.toolbar_height)
    }
    ...
    override fun onPrepareOptionsMenu(menu: Menu) {
        ...
        val postHasUrl = post?.hasUrl() == true
    ...
    private fun restoreState(savedInstanceState: Bundle?) {
        savedInstanceState?.let { state ->
            isFeed = state.getBoolean(ReaderConstants.ARG_IS_FEED)
            blogId = state.getLong(ReaderConstants.ARG_BLOG_ID)
            postId = state.getLong(ReaderConstants.ARG_POST_ID)
            directOperation = state
                    .getSerializable(ReaderConstants.ARG_DIRECT_OPERATION) as? DirectOperation
            commentId = state.getInt(ReaderConstants.ARG_COMMENT_ID)
            isRelatedPost = state.getBoolean(ReaderConstants.ARG_IS_RELATED_POST)
            interceptedUri = state.getString(ReaderConstants.ARG_INTERCEPTED_URI)
            postSlugsResolutionUnderway = state.getBoolean(ReaderConstants.KEY_POST_SLUGS_RESOLUTION_UNDERWAY)
            hasAlreadyUpdatedPost = state.getBoolean(ReaderConstants.KEY_ALREADY_UPDATED)
            hasAlreadyRequestedPost = state.getBoolean(ReaderConstants.KEY_ALREADY_REQUESTED)
            hasTrackedGlobalRelatedPosts = state.getBoolean(ReaderConstants.KEY_ALREADY_TRACKED_GLOBAL_RELATED_POSTS)
            hasTrackedLocalRelatedPosts = state.getBoolean(ReaderConstants.KEY_ALREADY_TRACKED_LOCAL_RELATED_POSTS)
            if (state.containsKey(ReaderConstants.ARG_POST_LIST_TYPE)) {
                this.postListType = state.getSerializable(ReaderConstants.ARG_POST_LIST_TYPE) as ReaderPostListType
            }
            if (state.containsKey(ReaderConstants.KEY_ERROR_MESSAGE)) {
                errorMessage = state.getString(ReaderConstants.KEY_ERROR_MESSAGE)
            }
        }
    }
    ...
    private fun togglePostLike() {
        post?.let {
            setPostLike(!it.isLikedByCurrentUser)
        }
    }
    ...
    private fun toggleBookmark() {
        ...
        post = ReaderPostTable.getBlogPost(post!!.blogId, post!!.postId, false)
    ...
    private fun showBookmarksSavedLocallyDialog() {
        ...
        basicFragmentDialog.show(fragmentManager!!, BOOKMARKS_SAVED_LOCALLY_DIALOG)
    }
    ...
    private fun showRelatedPosts(relatedPosts: ReaderSimplePostList, isGlobal: Boolean) {
        ...
        val listener = ReaderSimplePostView.OnSimplePostClickListener { v, siteId, postId ->
     ...
     * that the view is a child of scrollView
     */
    ...
    private fun doLikePost() {
        ...
        if (!accountStore.hasAccessToken()) {
            WPSnackbar.make(
                    ...
                    .setAction(R.string.sign_in, signInClickListener).show()
            return
        }
    ...
    private inner class ShowPostTask : AsyncTask<Void, Void, Boolean>() {
        ...
        override fun onPostExecute(result: Boolean) {
            ...
            renderer = ReaderPostRenderer(readerWebView, post)
    ...
    private fun downloadFile(fileUrl: String?, activity: FragmentActivity) {
        ...
        val fileName = URLUtil.guessUrl(fileUrl)
    ...
    private fun canShowBookmarkButton(): Boolean {
        return post?.isDiscoverPost == false
    }
