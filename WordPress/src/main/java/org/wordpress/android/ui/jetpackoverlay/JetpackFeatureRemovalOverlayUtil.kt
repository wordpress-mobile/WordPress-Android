...
class JetpackFeatureRemovalOverlayUtil @Inject constructor(
    ...
    /**
     * The app-open overlay tells the user their site has the Jetpack plugin and that its features moved to the
     * Jetpack app, so it's only shown for a site that can actually use that app. Sites without Jetpack don't
     * mark the overlay as seen, so it still appears the first time one that can is selected.
     */
