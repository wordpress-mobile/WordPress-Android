...
class SubfilterListItemMapper @Inject constructor(
    ...
    fun fromJson(
        ...
        val organization = Organization.fromOrgId(mappedItem.organization)
        ...
        return when (mappedItem.type) {
            SITE.value -> {
                ...
                } ?: SiteAll(isSelected = isSelected, organization = organization)
            }
            TAG.value -> {
            ...
            SITE_ALL.value -> SiteAll(isSelected = isSelected, organization = organization)
    ...
    fun toJson(item: SubfilterListItem): String {
        ...
        val mappedItem = MappedSubfilterListItem(type = item.type.value, organization = item.organization.orgId)
