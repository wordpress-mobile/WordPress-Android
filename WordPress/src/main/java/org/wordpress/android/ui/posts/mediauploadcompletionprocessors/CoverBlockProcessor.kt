    ...
    override fun processBlockContentDocument(document: Document): Boolean {
        ...
        if (targetDiv != null) {
            if (hasVideoBackground) {
                ...
                val style = PATTERN_BACKGROUND_IMAGE_URL.matcher(targetDiv.attr("style"))
                    .replaceFirst(String.format("background-image:url(%1\$s)", remoteUrl))
    ...
    companion object {
        ...
        private val PATTERN_COVER_INNER: Pattern = Pattern.compile(
            StringBuilder()
                ...
                .append("(\\s*</div>\\s*</div>\\s*<!-- /wp:cover -->.*)").toString(),
            Pattern.DOTALL
        )
         ...
         */
        private val PATTERN_BACKGROUND_IMAGE_URL: Pattern = Pattern.compile("background-image:\\s*url\\([^)]+\\)")
    }
}
