...
fun RevealRow(
    ...
    LaunchedEffect(revealId) {
        ...
        // Disarm either way. A refresh replaces the list with page 1 only, so a row that sorts
        // beyond it never arrives here; leaving the request armed would fire it much later, when
        // load-more finally paged it in and the user was reading something else.
        onRevealHandled()
    }
}
