...
class ThemeBrowserFragment : Fragment(), AbsListView.RecyclerListener,
    ...
    override fun onResume() {
        super.onResume()
        (activity as? ScrollableViewInitializedListener)?.onScrollableViewInitialized(requireNotNull(gridView).id)
    }
    ...
    private fun configureSwipeToRefresh(view: View) {
        swipeToRefreshHelper = WPSwipeToRefreshHelper.buildSwipeToRefreshHelper(view.findViewById(R.id.ptr_layout)) {
            ...
            if (!NetworkUtils.checkConnection(activity)) {
                ...
                emptyTextView?.setText(R.string.no_network_title)
                return@buildSwipeToRefreshHelper
            }
            setRefreshing(true)
    ...
    private fun addMainHeader(inflater: LayoutInflater) {
        ...
        details.setOnClickListener { v: View? -> callback?.onDetailsSelected(currentThemeId) }
        val support = header.findViewById<LinearLayout>(R.id.support)
        support.setOnClickListener { v: View? -> callback?.onSupportSelected(currentThemeId) }
    ...
    private fun updateDisplay() {
        ...
        emptyView?.visibility = if (!hasThemes) View.VISIBLE else View.GONE
        if (!hasThemes && !NetworkUtils.isNetworkAvailable(activity)) {
            emptyTextView?.setText(R.string.no_network_title)
        }
        gridView?.visibility = if (hasVisibleThemes) View.VISIBLE else View.GONE
        actionableEmptyView?.visibility = if (hasNoMatchingThemes) View.VISIBLE else View.GONE
    }
