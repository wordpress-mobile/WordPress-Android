...
public class TiledGalleryBlockProcessor extends BlockProcessor {
    ...
    /**
     * Query selector for selecting the img element from the tiled gallery which needs processing
     */
    private String mTiledGalleryImageQuerySelector;

    /**
     * Template pattern used to match and splice inner image blocks in the refactored gallery format
     */
    private static final Pattern PATTERN_TILED_GALLERY_INNER = Pattern.compile(new StringBuilder()
            .append("(^.*?<div class=\"[^\"]*?wp-block-jetpack-tiled-gallery[^\"]*?\">\\s*)")
            .append("(.*)") // inner block contents
            .append("(\\s*</div>\\s*<!-- /wp:jetpack/tiled-gallery -->.*)").toString(), Pattern.DOTALL);

    public TiledGalleryBlockProcessor(String localId, MediaFile mediaFile, String siteUrl, MediaUploadCompletionProcessor
            mediaUploadCompletionProcessor) {
        super(localId, mediaFile);
        mMediaUploadCompletionProcessor = mediaUploadCompletionProcessor;
        mTiledGalleryImageQuerySelector = new StringBuilder()
                .append("img[data-id=\"")
                .append(localId)
                .append("\"]")
                .toString();
    ...
    @Override boolean processBlockContentDocument(Document document) {
        ...
        Element targetImg = document.select(mTiledGalleryImageQuerySelector).first();
    ...
    @Override String processInnerBlock(String block) {
        Matcher innerMatcher = PATTERN_TILED_GALLERY_INNER.matcher(block);
