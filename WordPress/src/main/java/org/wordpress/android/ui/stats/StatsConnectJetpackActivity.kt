...
class StatsConnectJetpackActivity : BaseAppCompatActivity() {
    ...
    /**
     * This screen pitches installing Jetpack, which is wrong for a site that already has it. Such a site is
     * here because its Jetpack connection belongs to another WordPress.com account, or to no account yet, so
     * what it needs is the connection flow -- whose install step recognises Jetpack is present and moves on.
     * Skip straight to it.
     *
