...
class NotificationsPermissionBuilder @Inject constructor() {
    fun buildUiItems(appName: String, showAppSettingsGuide: Boolean): List<BloggingRemindersItem> {
        val title = UiStringRes(R.string.blogging_reminders_notifications_permission_title)

        val body = UiString.UiStringResWithParams(
            R.string.blogging_reminders_notifications_permission_description,
    ...
    fun buildPrimaryButton(onDone: () -> Unit): PrimaryButton {
        return PrimaryButton(
            UiStringRes(R.string.blogging_reminders_notifications_permission_primary_button),
            enabled = true,
