...
class TrafficBarChartViewHolder(parent: ViewGroup) : BlockListItemViewHolder(
    ...
    private fun BarChart.draw(
        item: BlockListItem.BarChartItem,
        labelStart: TextView,
        labelEnd: TextView
    ): BarCount {
        resetChart()
    ...
    private fun buildDataSet(context: Context, cut: List<BarEntry>): BarDataSet {
        ...
        dataSet.color = ContextCompat.getColor(context, R.color.wordpress_blue_40)
        dataSet.setGradientColor(
            ContextCompat.getColor(
                context,
                R.color.wordpress_blue_40
            ), ContextCompat.getColor(
                context,
                R.color.wordpress_blue_40
            )
    ...
    private fun toBarEntry(bar: BlockListItem.BarChartItem.Bar, index: Int): BarEntry {
        return BarEntry(
            index.toFloat(),
            bar.value.toFloat(),
            bar.id
