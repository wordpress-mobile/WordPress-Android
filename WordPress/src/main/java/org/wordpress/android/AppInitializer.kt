...
import org.wordpress.android.analytics.AnalyticsTracker.Stat.ACCOUNT_LOGOUT
import org.wordpress.android.analytics.AnalyticsTracker.Stat.APPLICATION_CLOSED
import org.wordpress.android.analytics.AnalyticsTracker.Stat.APPLICATION_INSTALLED
import org.wordpress.android.analytics.AnalyticsTracker.Stat.APPLICATION_OPENED
import org.wordpress.android.analytics.AnalyticsTracker.Stat.APPLICATION_UPGRADED
...
import org.wordpress.android.ui.posts.editor.ImageEditorInitializer.Companion.init
...
import org.wordpress.android.util.AppThemeUtils.Companion.setAppTheme
...
import org.wordpress.android.util.QuickStartUtils.cancelQuickStartReminder
...
class AppInitializer @Inject constructor(
    ...
    fun init() {
        ...
        init(imageManager, imageEditorTracker, imageEditorFileUtils, appScope)
    ...
    */
    ...
    private fun enableLogRecording() {
        ...
        AppLog.i(UTILS, "AppInitializer.init")
    }
    ...
    private fun createNotificationChannelsOnSdk26() {
        ...
        if (VERSION.SDK_INT >= VERSION_CODES.O) {
            ...
            val normalChannel = NotificationChannel(
                    application.getString(string.notification_channel_normal_id),
                    application.getString(string.notification_channel_general_title),
            ...
            val importantChannel = NotificationChannel(
                    application.getString(string.notification_channel_important_id),
                    application.getString(string.notification_channel_important_title),
            ...
            val reminderChannel = NotificationChannel(
                    application.getString(string.notification_channel_reminder_id),
                    application.getString(string.notification_channel_reminder_title),
            ...
            val transientChannel = NotificationChannel(
                    application.getString(string.notification_channel_transient_id),
                    application.getString(string.notification_channel_transient_title),
                    NotificationManager.IMPORTANCE_DEFAULT
            )
    ...
    private fun initAnalytics(elapsedTimeOnCreate: Long) {
        ...
        if (oldVersionCode == 0) {
            ...
            AnalyticsTracker.track(APPLICATION_INSTALLED)
        }
        if (oldVersionCode != 0 && oldVersionCode < versionCode) {
            ...
            // app upgraded
            AnalyticsTracker.track(APPLICATION_UPGRADED, properties)
        }
    ...
    private fun initWpDb() {
        if (!createAndVerifyWpDb()) {
            AppLog.e(DB, "Invalid database, sign out user and delete database")
            // Force DB deletion
    ...
    fun wordPressComSignOut() {
        ...
        AnalyticsTracker.track(ACCOUNT_LOGOUT)
        ...
        if (!event.isError && accountStore.get().hasAccessToken()) {
            ...
            if (event.causeOfChange == FETCH_ACCOUNT) {
                ...
                if (thisUserId != lastUserId) {
                    ...
                    AppLog.i(READER, "User changed, resetting reader db")
    ...
    fun onUnexpectedError(event: OnUnexpectedError) {
        AppLog.d(API, "Receiving OnUnexpectedError event, message: " + event.exception.message)
    }
    ...
    private fun removeWpComUserRelatedData(context: Context) {
        ...
        if (!BuildConfig.IS_JETPACK_APP || siteStore.hasSite()) {
    ...
    private fun enableHttpResponseCache(context: Context) {
        try {
            ...
            AppLog.w(UTILS, "Failed to enable http response cache")
        }
    }
    ...
    inner class ApplicationLifecycleMonitor {
        ...
        fun onAppGoesToBackground() {
            AppLog.i(UTILS, "App goes to background")
            ...
            AnalyticsTracker.track(APPLICATION_CLOSED, properties)
            AnalyticsTracker.endSession(false)
        ...
        fun onAppComesFromBackground() {
            ...
            AppLog.i(UTILS, "App comes from background")
            if (!WordPress.sAppIsInTheBackground) {
                return
            }
