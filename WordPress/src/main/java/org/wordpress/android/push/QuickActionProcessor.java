...
class QuickActionProcessor {
    ...
    private void getNoteFromBundleIfExists() {
        if (mIntent.hasExtra(NotificationsProcessingService.ARG_NOTE_BUNDLE)) {
            Bundle payload = mIntent.getBundleExtra(NotificationsProcessingService.ARG_NOTE_BUNDLE);
    ...
    private void getDataFromIntent() {
        ...
        if (mIntent.hasExtra(NotificationsProcessingService.ARG_ACTION_REPLY_TEXT)) {
            mReplyText = mIntent.getStringExtra(NotificationsProcessingService.ARG_ACTION_REPLY_TEXT);
        }
    ...
    public void requestCompleted(String actionType) {
        ...
        if (actionType != null) {
            ...
            successMessage = mNotificationsProcessingService.getString(R.string.comment_q_action_done_generic);
        }
    ...
    private void approveComment() {
        ...
        // Push the comment
        mDispatcher
                .dispatch(CommentActionBuilder.newPushCommentAction(new RemoteCommentPayload(site, comment)));
    }
    ...
    private void startReplyToCommentActivity() {
        ...
        mNotificationsProcessingService.startActivity(intent);
    }
