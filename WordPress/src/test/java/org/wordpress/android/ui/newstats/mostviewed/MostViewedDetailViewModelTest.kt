...
class MostViewedDetailViewModelTest : BaseUnitTest() {
    ...
    fun `when load succeeds, then loaded state with items and totals is emitted`() = test {
        whenever(detailFetcher.fetch(any(), any(), any(), any())).thenReturn(createSuccessResult())
    ...
    fun `when no site is selected, then error state is emitted and no fetch happens`() = test {
        ...
        verify(detailFetcher, never()).fetch(any(), any(), any(), any())
    }
    ...
    @Test
    fun `when the fetcher returns an error, then the api error state is emitted`() = test {
        whenever(detailFetcher.fetch(any(), any(), any(), any()))
            .thenReturn(StatsCardFetchResult.Error(R.string.stats_error_api))
    ...
    @Test
    fun `when the fetch throws, then the exception message is surfaced as the error state`() = test {
        whenever(detailFetcher.fetch(any(), any(), any(), any()))
    ...
    fun `when load is called twice, then the data is fetched only once`() = test {
        ...
        verify(detailFetcher, times(1)).fetch(any(), any(), any(), any())
    }
    ...
    fun `when retry is called after a load, then the data is fetched again`() = test {
        ...
        verify(detailFetcher, times(2)).fetch(any(), any(), any(), any())
    }

    private fun createSuccessResult() = StatsCardFetchResult.Success(
        items = listOf(
            MostViewedDetailItem(
                ...
                children = listOf(
                    MostViewedChildItem(name = "Google Search", url = CHILD_URL, views = TEST_VIEWS)
                )
            )
        ),
        totalValue = TEST_TOTAL_VIEWS,
        totalValueChange = TEST_TOTAL_VIEWS_CHANGE,
        totalValueChangePercent = TEST_TOTAL_VIEWS_CHANGE_PERCENT
    )
