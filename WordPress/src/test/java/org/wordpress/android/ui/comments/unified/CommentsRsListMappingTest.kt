...
class CommentsRsListMappingTest {
    @Test
    fun `toRsCommentListItem maps the fields the list rows need`() {
        val item = rsComment().toRsCommentListItem()
    ...
    @Test
    fun `toRsCommentListItem maps hold status to unapproved`() {
        val item = rsComment(status = RsCommentStatus.Hold).toRsCommentListItem()
