...
object MetadataUtils {
    fun getMetadata(attrs: AttributesWithClass, naturalWidth: Int, naturalHeight: Int): JSONObject {
        ...
        addMetadataProperty(
                ...
                "align",
                "none"
        ) // Accepted values: center, left, right or empty string.
        ...
        addMetadataProperty(
                ...
                "linkTargetBlank",
                false
        ) // true if the link should open in a new window.
        ...
        for (clazz in attrs.classes) {
            if (Pattern.matches("^wp-image.*", clazz)) {
                ...
                if (NumberUtils.toInt(attachmentIdString) != 0) {
                    ...
                    AppLog.d(
                            EDITOR,
                            "AttachmentId was not an integer! String value: $attachmentIdString"
                    )
                }
    ...
    private fun addMetadataProperty(
    ...
    fun getClassAttribute(attributes: Attributes): MutableSet<String?> {
        return if (attributes.getIndex("class") == -1) {
            HashSet(ArrayList())
        } else HashSet(
                Arrays.asList(
                        *attributes.getValue(
