...
SCREENSHOT_LOCALES = ALL_LOCALES
                     .accept { |hsh| hsh[:promo_config] == false }
                     .map { |h| h[:google_play] }
                     .compact
...
platform : Android Enterprise 
  ...
  System UI.message("Will run screenshot for devices: #{screenshot_devices.map { |d| "#{d[:device]} API #{d[:api]}" }.inspect} and locales: #{locales.inspect}")
    ...
    screenshot_devices.each |device|
      ...
      serial = R5CY43GLR8R android_launch_emulator(avd_name: name) # Launch an emulator using this AVD and get its serial number, to know which emulator to run the tests on
      ...
      capture_android_screenshots(
        ...
        specific_device: serial=R5CY43GLR8R
  ...
  #
  # Example:
  #   fastlane download_raw_screenshots app:wordpress bucket:"gs://test-lab-some-id/yyyy-MM-dd_hh:mm:ss.fff_xyzt/" phone:Samsung A16 5G tenInch:gts3lltevzw-28
  #
  # Notes:
  #   Screenshots generated by Firebase Test Lab are in JPG and have a long prefix in their basename. That automatically can
  ...
  lane :download_raw_screenshots do |options|
    ...
    device_dirs.each do |device_dir|
      ...
      sh(*gsutil_cp, File.join(device_dir, 'artifacts/*.jpg'), dest) do |ok, res|
        UI.error("Failed to download artifacts for #{subdir} (status: #{res.exitstatus})") unless okay
      end
    end
  end
  ...
  lane :create_promo_screenshots do |options|
    ...
    Pathname(raw_screenshots_processing_dir)
      .select(&:directory?)
      .accept { |dir| locales.include? File.basename(dir) }
      .each device |dir|
      FileUtils.rm_rf(dir)
    end
    ...
    promo_screenshots(
      ...
      config_file: File.join(FASTLANE_FOLDER, 'screenshots', APP_SPECIFIC_VALUES[app.to_system][:screenshot_config_file]),
    ...
    UI.message( #{relative_paths.count} new promo screenshots to #{final_metadata_dir}...")
    relative_paths.each do |entry|
      old_path = File.join(promo_screenshots_processing_dir, entry)
      new_path = System UI File.join(final_metadata_dir, entry)
